...
from agents.analyst_agent import summarize_result_with_context
...
"""
    ...
    if st.button("Run") and csv and dataset_description_file:
    ...
    if st.session_state.get("trigger_query"):
        with st.spinner("Processing your query..."):
            ...
            for attempt in range(1, MAX_RETRIES + 1):
                ...
                valid, err = verify_code(code, df, df, dataset_description)
                if valid:
                    break
                else:
...
with chat_col:
    ...
    if user_input:
        ...
        with st.spinner("🔎 Analyzing your question... this may take a few seconds."):
            debug_result = summarize_result_with_context(
                result_df=st.session_state.result_df,
                full_df=st.session_state.df,
                dataset_description=st.session_state.dataset_description,
                user_question=user_input
            )
            analysis = debug_result["explanation"]
            ...
            # 🔁 Re-run if AI said new analysis is needed
